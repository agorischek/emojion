...
var lodash_1 = lodash.invert;
var lodash_2 = lodash.merge;
var lodash_3 = lodash.forEach;
var lodash_4 = lodash.forOwn;
...
function makeRegexp(characters) {
...
const flip = (dict) => {
    const flipped = lodash_1(dict);
    return flipped;
};
const flipMultiple = (dicts) => {
    const flippedArray = dicts.map(x => flip(x));
    const merged = flippedArray.reduce((acc, val) => lodash_2(acc, val));
    return merged;
};
...
const lookupTable = flipMultiple([
...
const keyUpperLetters = {};
lodash_3(characters.keys.letters, (value, key) => {
    keyUpperLetters[key.toUpperCase()] =
        characters.keys.modifiers.upperCase + value;
});
const valueUpperLetters = {};
lodash_3(characters.values.letters, (value, key) => {
    valueUpperLetters[key.toUpperCase()] =
        characters.values.modifiers.upperCase + value;
});
const conversionTable = {
    keys: lodash_2({}, characters.keys.letters, keyUpperLetters, characters.keys.numerals),
    strings: lodash_2({}, characters.values.letters, valueUpperLetters, characters.values.numerals),
    numbers: characters.values.numbers,
};

const lexer = moo_1(patterns);
const lex = (doc) => {
    lexer.reset(doc);
...
var nearley_1 = nearley.Grammar;
var nearley_2 = nearley.Parser;

const lookUp = (character) => lookupTable[character];
const convert = {
    key: (character) => conversionTable.keys[character],
    value: (character) => conversionTable.strings[character],
    number: (character) => conversionTable.numbers[character],
};
...
const CONVERTUPPER = (d) => {
    return lookUp(d[1]).toUpperCase();
};
const CONCAT = (d) => {
    return d[0].join('');
};
const CONCATWRAPSTRING = (d) => {
    return '"' + d[0].join('') + '"';
};
const SELF = (d) => {
    return d[0];
...
const compiledGrammar = nearley_1.fromCompiled(grammar);
const parser = new nearley_2(compiledGrammar);
const parse = (input) => {
    const wrappedParser = new nearley_2(compiledGrammar);
...
const generate = (input) => {
    ...
    const generated = characters.document.start +
        processDocument(parsed) +
        characters.document.end;
    return generated;
};
const processDocument = (input) => {
...
const processObject = (input, isRoot) => {
    const convertedProperties = [];
    lodash_4(input, (value, key) => {
    ...
    if (isRoot) {
        ...
        return (characters.document.indent +
            convertedProperties +
            characters.document.outdent);
    }
};
const processProperty = (input) => {
...
const processArray = (input) => {
    ...
    const generated = characters.document.arrayOpen + converted + characters.document.arrayClose;
    return generated;
};
